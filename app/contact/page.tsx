...
export default function ContactPage() {
  ...
  const submit = async (data: ContactForm) => {
    const mailto = `mailto:${contactDetails.email}?subject=Alaira enquiry from ${encodeURIComponent(data.name)}&body=${encodeURIComponent(`${data.message}\n\nPhone: ${data.phone}\nEmail: ${data.email}`)}`;
    ...
    <div className="bg-[var(--color-ivory)] pt-24">
      <div className="container-page py-14">
        ...
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <aside className="rounded-lg border border-[rgba(201,168,76,0.28)] bg-[var(--color-dark-red)] p-5 text-[var(--color-ivory)] sm:p-6">
            <h2 className="font-display text-3xl font-semibold sm:text-4xl">Alaira Half Saree House</h2>
            <div className="mt-6 grid gap-4 text-sm leading-6 text-[rgba(250,243,224,0.76)]">
              <a href={`mailto:${contactDetails.email}`} className="flex min-w-0 gap-3 break-all hover:text-[var(--color-gold-soft)]"><Mail className="mt-1 shrink-0 text-[var(--color-gold)]" size={18} /> {contactDetails.email}</a>
              <a href={`tel:${contactDetails.phone.replace(/\s/g, "")}`} className="flex gap-3 hover:text-[var(--color-gold-soft)]"><Phone className="mt-1 text-[var(--color-gold)]" size={18} /> {contactDetails.phone}</a>
              <span className="flex gap-3"><MapPin className="mt-1 shrink-0 text-[var(--color-gold)]" size={18} /> {contactDetails.addressLines.join(" ")}</span>
              <a href={contactDetails.instagramUrl} target="_blank" rel="noreferrer" className="flex gap-3 hover:text-[var(--color-gold-soft)]"><Camera className="mt-1 shrink-0 text-[var(--color-gold)]" size={18} /> {contactDetails.instagramHandle}</a>
              <p className="border-t border-[rgba(201,168,76,0.18)] pt-4">Working hours: {contactDetails.hours}</p>
            </div>
