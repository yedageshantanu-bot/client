    ...
    <>
      ...
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          {/* Brand logo */}
          <Link href="/" className="relative z-10 flex items-center gap-3 rounded-full pr-3 transition duration-500 hover:bg-white/10" aria-label="Alaira House Home">
            <BrandLogo
              priority
              className={cn(
                "transition duration-500",
                "bg-[#1C1924] text-white",
              )}
            />
            <span className="font-display text-lg font-bold uppercase tracking-[0.14em] text-[var(--color-brown)] transition hover:text-[var(--color-maroon)] sm:text-xl">
              {brandName}
            </span>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  className={cn(
                    "group relative py-2 font-display text-[0.88rem] font-medium transition duration-300 hover:-translate-y-0.5 whitespace-nowrap",
                    "text-[#1C1924] hover:text-[var(--color-maroon)]",
                    active && "text-[var(--color-maroon)] font-semibold",
                  )}
                >
                  {link.label}
                  <span className="absolute -bottom-1 left-1/2 h-px w-0 -translate-x-1/2 bg-[linear-gradient(90deg,transparent,var(--color-maroon),transparent)] transition-all duration-300 group-hover:w-full" />
                </Link>
              );
            })}
          </nav>
